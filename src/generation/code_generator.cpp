...
{
    bool CodeGenerator::FindMatchingFunction( FunctionDefinition::Ref &function,
        ...
        const std::vector< FragmentDefinition::Ref > &definition_table )
    {
        ...
        for ( ; it != end; ++it )
        {
            if ( ( *it )->FindFunctionDefinitionMatchingSemanticSet( function, semantic_set ) )
            {
                ...
                if ( std::find( m_UsedTranslationUnitSet.begin(),
                         m_UsedTranslationUnitSet.end(),
                         &( *it )->GetTranslationUnit() ) == m_UsedTranslationUnitSet.end() )
                {
                    m_UsedTranslationUnitSet.push_back(
                        const_cast< AST::TranslationUnit * >( &( *it )->GetTranslationUnit() ) );
                }
    ...
    void CodeGenerator::RemoveInputSemantics( std::set< std::string > &semantic_set )
    {
        ...
        std::set_intersection( semantic_set.begin(),
            semantic_set.end(),
            m_InputSemanticSet.begin(),
            m_InputSemanticSet.end(),
            std::inserter( m_UsedSemanticSet, m_UsedSemanticSet.begin() ) );

        std::set_difference( semantic_set.begin(),
            semantic_set.end(),
            m_InputSemanticSet.begin(),
            m_InputSemanticSet.end(),
    ...
    Base::ObjectRef< AST::Statement > GetFunctionCallFromFunctionDefinition( const FunctionDefinition &definition )
    {
        ...
        Base::ObjectRef< AST::CallExpression > call_expression =
            new AST::CallExpression( function_declaration.m_Name, &*argument_expression_list );
    ...
    struct CodeGeneratorHelper
    {
        void Visit( const GraphNode &node )
        {
            ...
            for ( ; it != end; ++it )
            {
                if ( m_DeclaredVariableTable.find( *it ) == m_DeclaredVariableTable.end() )
                {
                    ...
                    Base::ObjectRef< AST::VariableDeclarationStatement > variable =
        ...
        std::set< std::string > m_DeclaredVariableTable;
        std::set< const GraphNode * > m_VisitedNodeSet;
        std::vector< Base::ObjectRef< AST::Statement > > m_StatementTable;
        std::map< std::string, std::string > m_SemanticToTypeTable;
    };

    void CodeGenerator::AddArgumentTable( AST::ArgumentList &argument_list,
    ...
    Base::ObjectRef< AST::FunctionDeclaration > CodeGenerator::GenerateCodeFromGraph( const Graph &graph )
    {
        ...
        std::set< std::string > used_input_semantic_set, input_semantic_set, output_semantic_set,
            input_output_semantic_set;

        std::set_intersection( m_InputSemanticSet.begin(),
            m_InputSemanticSet.end(),
            m_UsedSemanticSet.begin(),
            m_UsedSemanticSet.end(),
            std::inserter( used_input_semantic_set, used_input_semantic_set.begin() ) );

        std::set_intersection( used_input_semantic_set.begin(),
            used_input_semantic_set.end(),
            m_OutputSemanticSet.begin(),
            m_OutputSemanticSet.end(),
            std::inserter( input_output_semantic_set, input_output_semantic_set.begin() ) );

        std::set_difference( used_input_semantic_set.begin(),
            used_input_semantic_set.end(),
            m_OutputSemanticSet.begin(),
            m_OutputSemanticSet.end(),
            std::inserter( input_semantic_set, input_semantic_set.begin() ) );

        std::set_difference( m_OutputSemanticSet.begin(),
            m_OutputSemanticSet.end(),
            used_input_semantic_set.begin(),
            used_input_semantic_set.end(),
    ...
    Graph::Ref CodeGenerator::GenerateGraph( const std::vector< FragmentDefinition::Ref > &fragment_table )
    {
        std::set< std::string > open_set, closed_set;
        Graph::Ref graph;
        ...
        while ( !open_set.empty() )
        {
            ...
            for ( ; it != end; ++it )
            {
                if ( open_set.find( *it ) == open_set.end() && graph->HasGeneratedSemantic( *it ) )
                {
            ...
            std::set_difference( open_set.begin(),
                open_set.end(),
                function->GetOutSemanticSet().begin(),
                function->GetOutSemanticSet().end(),
    ...
    void CodeGenerator::GenerateShader( Base::ObjectRef< AST::TranslationUnit > &generated_shader,
    ...
    void CodeGenerator::MergeTranslationUnit( AST::TranslationUnit &destination_translation_unit,
        const std::vector< Base::ObjectRef< AST::TranslationUnit > > &translation_unit_table )
    {
        ...
        for ( ; it != end; ++it )
        {
            ...
            std::copy( clone->m_GlobalDeclarationTable.begin(),
                clone->m_GlobalDeclarationTable.end(),
                std::back_inserter( destination_translation_unit.m_GlobalDeclarationTable ) );
        }
    }

    bool CodeGenerator::ValidatesGraph( const Graph &graph ) const
    {
        GraphValidator validator( *m_ErrorHandler );
