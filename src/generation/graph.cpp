...
namespace Generation
{
    ...
    {
        auto it = semantic_set.begin();
        auto end = semantic_set.end();
    ...
    {
        std::set< std::string > output_semantic, input_semantic;
        ...
        for ( ; it != end; ++it )
        {
            ...
            using node_iterator = std::multimap< std::string, Base::ObjectRef< GraphNode > >::iterator;
    ...
    bool Graph::HasGeneratedSemantic( const std::string &semantic ) const
    {
        return m_NodeToLastOutputSemanticMap.find( semantic ) != m_NodeToLastOutputSemanticMap.end();
    }

    bool Graph::UseGeneratedSemantic( GraphNode &node, const std::string &semantic )
    {
        std::map< std::string, GraphNode::Ref >::iterator generating_node;
