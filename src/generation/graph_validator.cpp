...
namespace Generation
{
    ...
    GraphValidator::GraphValidator( Base::ErrorHandlerInterface &error_handler )
        : m_ErrorHandler( &error_handler ), m_GraphHasErrors( false )
    {
    }
    ...
    void GraphValidator::Visit( const GraphNode &node )
    {
        ...
        const FunctionDefinition &definition = node.GetFunctionDefinition();
        ...
        for ( ; it != end; ++it )
        {
            ...
            else
            {
                if ( ( *value ).second != ( *it ).second )
                {
                    ...
                    message << "Incompatible type for semantic " << ( *it ).first << " in function "
                            << definition.GetName() << ": previously seen type was " << ( *value ).second
                            << " but defined here as " << ( *it ).second;
