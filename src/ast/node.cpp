...
#define CloneTable( _Type_, _member_ )                                                                                 \
    ...
    Node::Node() : m_FileName( s_CurrentFileName ), m_Line( s_CurrentLine )
    {
    }

    void Node::SetDebugInfo( const std::string &filename, const int line )
    {
        s_CurrentFileName = filename;
        s_CurrentLine = line;
    }

    std::string Node::s_CurrentFileName;
    int Node::s_CurrentLine = -1;

    TranslationUnit *TranslationUnit::Clone() const
    {
        TranslationUnit *clone = new TranslationUnit;

        CloneTable( GlobalDeclaration, m_GlobalDeclarationTable ) CloneTable( Technique, m_TechniqueTable )
