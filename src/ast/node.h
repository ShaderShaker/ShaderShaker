...
#define AST_HandleVisitor()                                                                                            \
    void Visit( AST::Visitor &visitor ) override                                                                       \
    {                                                                                                                  \
        visitor.Visit( *this );                                                                                        \
    }                                                                                                                  \
    void Visit( AST::ConstVisitor &visitor ) const override                                                            \
    {                                                                                                                  \
        visitor.Visit( *this );                                                                                        \
    }
    ...
    {
        Node();
        virtual void Visit( AST::Visitor &visitor )
        {
            visitor.Visit( *this );
        };
        virtual void Visit( AST::ConstVisitor &visitor ) const
        {
            visitor.Visit( *this );
        };
        ...
        static void SetDebugInfo( const std::string &filename, const int line );
    ...
    {
        AST_HandleVisitor() void SetType( Type *type )
        {
            assert( type );
            m_Type = type;
        }
    ...
    struct TextureDeclaration : GlobalDeclaration
    {
        ...
        TextureDeclaration( const std::string &type,
            ...
            : m_Type( type ), m_Name( name ), m_Semantic( semantic ), m_Annotations( annotations )
        {
        }
    ...
    struct StructDefinition : GlobalDeclaration
    {
        ...
        struct Member
        {
            ...
            Base::ObjectRef< Type > m_Type;
            std::string m_Name, m_Semantic, m_InterpolationModifier;
        };
        ...
        void AddMember( const std::string &name,
            ...
            const std::string &interpolation_modifier )
        {
            // :HACK: Visual studio 2010 does not support mutliple argument emplace_back, replace when fully C++11
            // compliant
