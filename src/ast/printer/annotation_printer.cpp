...
namespace AST
{
    ...
    {
        VisitorStreamArrayItemSeparator< Annotations::AnnotationTableType > separator( m_Stream );
    ...
    {
        if ( annotation_entry.m_Name.empty() || annotation_entry.m_Type.empty() || annotation_entry.m_Value.empty() )
        {
            return;
        }

        m_Stream << "\"" << annotation_entry.m_Name << "\":"
                 << "{"
