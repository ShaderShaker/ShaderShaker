...
struct HLSLParserTraits : public HLSLLexerTraits
{
    class RuleReturnValueType : public antlr3::RuleReturnValue< HLSLParserTraits >
    {
      ...
      public:
        RuleReturnValueType() : RuleReturnValue(), m_StartToken( 0 ), m_PreviousLine( -1 ), m_PreviousFile()
        {
        }
        ...
        {
            AST::Node::SetDebugInfo( m_StartToken->get_input()->get_fileName(), m_StartToken->get_line() );
        }
...
{
  public:
    using ImplTraits = TraitsBase< HLSLUserTraits >;
    using Callback = void ( * )( void *content, size_t &size, const char * );
